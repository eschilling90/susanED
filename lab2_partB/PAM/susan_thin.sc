...
behavior SusanThinThread(int r[IMAGE_SIZE], uchar mid[IMAGE_SIZE], in int thID, OS_API_TOP os_port)
{
    ...
    os_port.os_timewait(1, 6400000);
    }                
};

behavior SusanThin_ReadInput(i_int7220_receiver in_r, i_uchar7220_receiver in_mid, int r[IMAGE_SIZE], uchar mid[IMAGE_SIZE], OS_API_TOP os_port)
{

    void main(void) {
        in_r.receive(&r, 1, os_port);
        in_mid.receive(&mid, 1, os_port);
    }
};

behavior SusanThin_WriteOutput(i_uchar7220_sender out_mid, uchar mid[IMAGE_SIZE], OS_API_TOP os_port)
{
    void main(void) {
        out_mid.send(mid, 1, os_port);      
    }
};

behavior SusanThin(int r[IMAGE_SIZE], uchar mid[IMAGE_SIZE], OS_API_TOP os_port)
{
 
    SusanThinThread susan_thin_thread_0(r, mid, 0, os_port);
    SusanThinThread susan_thin_thread_1(r, mid, 1, os_port);
...
behavior Thin(i_int7220_receiver in_r, i_uchar7220_receiver in_mid, i_uchar7220_sender out_mid, OS_API_TOP os_port)
{

    int r[IMAGE_SIZE];
    uchar mid[IMAGE_SIZE];
 
    SusanThin_ReadInput susan_thin_read_input(in_r, in_mid, r, mid, os_port);
    SusanThin_WriteOutput susan_thin_write_output(out_mid, mid, os_port);   
    SusanThin susan_thin(r, mid, os_port);
    
    void main(void) {
	os_port.os_register(1);	// fixed threadID, ID of thin is 1
	os_port.acquire_running_key(1);
        susan_thin_read_input.main();
        susan_thin.main();
        susan_thin_write_output.main();
	os_port.os_terminate(1);
    }
    
};

// a fsm endless-cycle wapper
behavior ThinWapper(i_int7220_receiver in_r, i_uchar7220_receiver in_mid, i_uchar7220_sender out_mid, OS_API_TOP os_port)
{
  Thin inst(in_r, in_mid, out_mid, os_port);
