...
#ifndef OS_TYPED_SENDER_SH
#define OS_TYPED_SENDER_SH


#define DEFINE_OS_TYPED_SENDER(typename, type)				\
									\
interface i_ ## typename ## _sender					\
{									\
    note _SCE_STANDARD_LIB = { "i_sender", #typename, #type };		\
									\
    void send(type d, int threadID, OS_API_TOP os_port);		\
};


#endif /* OS_TYPED_SENDER_SH */
