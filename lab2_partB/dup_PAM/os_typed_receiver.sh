...
#ifndef OS_TYPED_RECEIVER_SH
#define OS_TYPED_RECEIVER_SH


#define DEFINE_OS_TYPED_RECEIVER(typename, type)			\
									\
interface i_ ## typename ## _receiver					\
{									\
    note _SCE_STANDARD_LIB = { "i_receiver", #typename, #type };	\
									\
    void receive(type *d, int threadID, OS_API_TOP os_port);		\
};


#endif /* OS_TYPED_RECEIVER_SH */
