...
/*
 * @intro: HardwareBusProtocolMACTLM channel for part 4
 */
channel HardwareBusProtocolMACTLM implements IMasterHardwareBusLinkAccess, ISlaveHardwareBusLinkAccess
{
  int a;
...
//printf("mac received\n");
    waitfor(39000 * IMAGE_SIZE / DATA_BYTES);
  }
  ...
  void MasterWrite(int addr, const void* data, unsigned long len)
  {
    a = addr;
    bus.send(data, len);
    waitfor(34000 * IMAGE_SIZE / DATA_BYTES);
  }
  ...
  void SlaveRead(int addr, void *data, unsigned long len)
  {
    while(a != addr)
      waitfor(1000);
    bus.receive(data, len);
//printf("slave finished read\n");
  }

  void SlaveWrite(int addr, const void* data, unsigned long len)
  {
//printf("slave issued write\n");
    while(a != addr)
      waitfor(1000);
    bus.send(data, len);
//printf("slave finished write\n");
  }
};
