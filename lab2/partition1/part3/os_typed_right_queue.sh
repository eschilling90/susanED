...
#ifndef C_TYPED_RIGHT_QUEUE_SH
#define C_TYPED_RIGHT_QUEUE_SH
...
#include "os_typed_sender.sh"
//#include "os_typed_receiver.sh"
#include "os_typed_right_tranceiver.sh"

import "full_rtos";

#define DEFINE_C_TYPED_QUEUE(typename, type)				\
									\
channel c_ ## typename ## _queue(in const unsigned long size, OS_API_TOP api_port)\
...
{									\
	...
	cleanup();							\
									...
									\
    void send(type d, int threadID)					\
    {									\
	while(n >= size)						\
	{								\
	    ws++;							\
	    api_port.pre_wait(threadID);				\
	    wait s;							\
	    api_port.post_wait(threadID);				\
	    ws--;							\
	}								\
									\
	setup();							\
