...
void mix_g(real_T fr_bw[19200], real_T bg_bw[19200], real_T fg[19200])
{
  int32_T k;
...
void save_frame1(real_T fr[WIDTH*HEIGHT], char base[], int idx) {
  FILE *pFile;
  char filename[32];
  int grayval, pix_idx;
...
behavior mixtures(i_receiver in_frame, i_sender fr_bg_bw, i_sender fr_fg) {

  real_T frame[HEIGHT*WIDTH];
  real_T bg_bw[HEIGHT*WIDTH];
  real_T fg[HEIGHT*WIDTH];

  unsigned char frg[HEIGHT*WIDTH];
  unsigned char frbg[HEIGHT*WIDTH];
  unsigned char frfg[HEIGHT*WIDTH];

  unsigned char rgbframe[HEIGHT*WIDTH*3];
  ...
  void main(void) {
    ...
    fprintf(stderr, "Initializing mix_g\n");
    mix_g_initialize();
    fprintf(stderr,"Initialized mix_g\n");

    fprintf(stderr, "Starting mix_g...\n");
    for(frame_idx=0; frame_idx < NUM_FRAMES; frame_idx++) {
      //receive input frame
      in_frame.receive(frg, sizeof(frg));
      
      for(q=0; q<19200; q++)
        frame[q] = (real_T) frg[q];

      // call the gausian mixtures algorithm
      mix_g(frame, bg_bw, fg);

      for(q=0; q < HEIGHT*WIDTH; q++) {
        frbg[q] = (int) bg_bw[q];
      }
      fr_bg_bw.send(frbg, sizeof(frbg));

      for(q=0; q < HEIGHT*WIDTH; q++) {
        frfg[q] = (int) fg[q];
      }
      fr_fg.send(frfg, sizeof(frfg));
